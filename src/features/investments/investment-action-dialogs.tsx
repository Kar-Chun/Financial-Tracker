...
export function PricesDialog({ accountId, currencyCode, today, holdings, open, onOpenChange }: CommonProps & { holdings: DetailedHolding[] }) {
  ...
  const submit=()=>{const updates=holdings.filter((item)=>!item.archived_at).map((item)=>({holding_id:item.id,price:normalizeInvestmentDecimal(prices[item.id]??"")}));if(updates.some((item)=>item.price===null)){setError("Enter a positive current price for every active holding within the database-supported precision.");return}mutation.mutate({accountId,pricedAt:date,prices:updates.filter(isValidPriceUpdate)},{onSuccess:()=>{toast.success("Holding prices updated.");onOpenChange(false)},onError:(cause)=>setError(getErrorMessage(cause,"Prices could not be updated."))})}
...
function isValidPriceUpdate(value:{holding_id:string;price:string|null}):value is {holding_id:string;price:string}{return value.price!==null}
