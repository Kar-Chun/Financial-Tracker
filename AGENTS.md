...
- Account archive requires a zero authoritative represented value and preserves history. Permanent deletion is only through the authenticated lifecycle RPC, must reject active transaction references, and must never cascade into unrelated accounts; historical net-worth snapshots are not retroactively rebuilt.
...
- Resetting Net Worth history may delete only the authenticated user's snapshot rows, must validate current Detailed values first, and must recreate today through the authoritative snapshot helper; it never resets financial records.
...
- `public.get_eligible_expense_facts` is the internal authoritative SQL source for ordinary eligible spending used by Analytics, Budgets, Dashboard, and AI read models. Keep consumer-specific date ranges and warning counts outside it; do not grant browser execution.
