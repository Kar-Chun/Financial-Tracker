...
export function DetailedInvestmentSetupPage() {
  ...
  return <section className="mx-auto w-full min-w-0 max-w-2xl">
    <header className="flex min-h-18 items-center gap-3 border-b border-border/25 pb-4"><Link to="/investments" aria-label="Back to investments" data-slot="button" className={buttonVariants({ variant: "ghost", size: "icon" })}><ArrowLeft /></Link><PageTitle eyebrow="One-way setup" title="Enable detailed tracking" /></header>
    <div className="form-fields py-5">
      ...
      {confirmedPreview && <div className="insight-surface p-4"><p className="section-heading">Conversion preview</p><dl className="mt-3 space-y-2 text-sm"><Row label="Current Simple value" value={formatCurrency(confirmedPreview.simple_native_value_minor, account.currency_code)} /><Row label="Detailed opening state" value={formatCurrency(confirmedPreview.detailed_native_value_minor, account.currency_code)} /><Row label="Difference" value={formatCurrency(confirmedPreview.difference_minor, account.currency_code)} /></dl><p className="mt-3 text-xs leading-5 text-muted-foreground">The difference is not adjusted automatically. Confirm only after checking cash, quantities, and prices.</p></div>}
...
function Row({ label, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-4"><dt className="text-muted-foreground">{label}</dt><dd className="max-w-[55%] break-words text-right font-medium tabular-nums [overflow-wrap:anywhere]">{value}</dd></div> }
