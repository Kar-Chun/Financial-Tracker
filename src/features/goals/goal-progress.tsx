...
export function GoalProgress({ goal, compact = false, linked = true }: { goal: SavingsGoalSummary; compact?: boolean; linked?: boolean }) {
  ...
  const content = (
    <div className={cn("min-w-0", !compact && "insight-surface p-4")}>
      <div className="flex min-w-0 items-start justify-between gap-3">
        <div className="min-w-0"><h3 className="break-words font-semibold text-foreground">{goal.name}</h3><p className="mt-1 break-words text-sm tabular-nums text-muted-foreground [overflow-wrap:anywhere]">{formatCurrency(goal.allocated_minor, goal.currency_code)} / {formatCurrency(goal.target_amount_minor, goal.currency_code)}</p></div>
  ...
  return linked ? <Link className="block rounded-xl transition-colors hover:bg-surface-elevated/40 outline-none focus-visible:ring-3 focus-visible:ring-ring" to={`/goals/${goal.id}`}>{content}</Link> : content
}
