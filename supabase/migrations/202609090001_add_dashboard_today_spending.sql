...
drop function public.get_dashboard_data();
drop function public.get_transactions_page(date, date, text, uuid, uuid, integer, date, timestamptz, uuid);
...
create function public.get_transactions_page(
  ...
  p_eligible_spending boolean default false
)
returns jsonb
language plpgsql
stable
...
begin
  ...
  if p_eligible_spending and (p_start_date is null or p_end_date is null) then
    raise exception using errcode = '22023', message = 'Eligible spending requires a bounded date range.';
  end if;
  ...
  with eligible_spending as materialized (
    select distinct fact.transaction_id
    from public.get_eligible_expense_facts(
      coalesce(p_start_date, date '1970-01-01'),
      coalesce(p_end_date, date '1970-01-01')
    ) fact
    where p_eligible_spending
    ...
    where transaction_record.user_id = v_user_id
      ...
      and (
        not p_eligible_spending
        or exists (
          select 1
          from eligible_spending eligible
          where eligible.transaction_id = transaction_record.id
        )
      )
      and (
...
declare
  ...
  v_today_spending_minor bigint := 0;
  v_seven_day_spending_minor bigint := 0;
  v_seven_day_average_minor bigint := 0;
  v_analytics jsonb;
  v_accounts jsonb;
...
begin
  ...
  select
    coalesce(sum(fact.amount_minor) filter (where fact.transaction_date = v_today), 0)::bigint,
    coalesce(sum(fact.amount_minor), 0)::bigint
  into v_today_spending_minor, v_seven_day_spending_minor
  from public.get_eligible_expense_facts(v_today - 6, v_today) fact;

  v_seven_day_average_minor := round(v_seven_day_spending_minor::numeric / 7)::bigint;
  ...
  return jsonb_build_object(
    ...
    'daily_spending', jsonb_build_object(
      'local_date', v_today,
      'today_minor', v_today_spending_minor,
      'seven_day_total_minor', v_seven_day_spending_minor,
      'seven_day_average_minor', v_seven_day_average_minor
    ),
...
revoke all on function public.get_transactions_page(date, date, text, uuid, uuid, integer, date, timestamptz, uuid, boolean)
  from public, anon;
grant execute on function public.get_transactions_page(date, date, text, uuid, uuid, integer, date, timestamptz, uuid, boolean)
  to authenticated;
