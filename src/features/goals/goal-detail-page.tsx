...
export function GoalDetailPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-6">
    <header className="flex items-center justify-between gap-3"><Link to="/goals" data-slot="button" className={buttonVariants({ variant: "ghost", size: "sm" })}><ArrowLeft /> Goals</Link><div className="flex gap-2"><Button variant="outline" size="sm" disabled={archived} onClick={() => setEditing(true)}><Pencil /> Edit</Button><Button variant="outline" size="sm" disabled={archiveMutation.isPending} onClick={setArchived}>{archived ? <RotateCcw /> : <Archive />}{archived ? "Restore" : "Archive"}</Button></div></header>
    ...
    <section className="min-w-0 border-b border-border/30 pb-5">
      ...
      <div className="mt-5 flex min-w-0 flex-wrap items-baseline gap-2"><span className="break-words font-serif text-[clamp(1.8rem,8vw,3rem)] [overflow-wrap:anywhere]">{formatCurrency(goal.allocated_minor, goal.currency_code)}</span><span className="text-muted-foreground">/ {formatCurrency(goal.target_amount_minor, goal.currency_code)}</span></div>
      ...
      {!goal.reached && goal.target_date && <div className="mt-6 insight-surface px-4 py-3"><p className="flex items-center gap-2 text-sm font-medium"><CalendarDays className="size-4 text-primary" /> Target {formatLongDate(goal.target_date)}</p><p className="mt-2 text-sm text-muted-foreground">{guidance.targetDatePassed ? "Target date passed" : guidance.requiredMonthlyMinor !== null ? `About ${formatCurrency(guidance.requiredMonthlyMinor, goal.currency_code)}/month needed` : ""}</p></div>}
      {!archived && <div className="mt-5 grid grid-cols-2 gap-3"><Button className="h-12" onClick={() => setAllocationOperation("allocate")}>Allocate</Button><Button className="h-12" variant="outline" disabled={goal.allocated_minor <= 0} onClick={() => setAllocationOperation("reduce")}>Reduce allocation</Button></div>}
    </section>

    <section className="space-y-3"><SectionHeader id="allocation-history" title="Allocation history" />{goal.allocations.length === 0 ? <div className="rounded-2xl bg-surface px-4 py-8 text-center text-sm text-muted-foreground ring-1 ring-border/25">No allocation history yet.</div> : <div className="border-y border-border/30">{goal.allocations.map((allocation) => <div key={allocation.id} className="flex min-w-0 items-center justify-between gap-3 border-b border-border/20 py-3 last:border-0"><div className="min-w-0"><p className="break-words font-medium">{allocation.note || (allocation.amount_minor > 0 ? "Allocation" : "Reduced allocation")}</p><p className="mt-1 text-sm text-muted-foreground">{formatShortDate(allocation.allocation_date)}</p></div><span className={cn("max-w-[45%] break-words text-right text-sm font-medium tabular-nums [overflow-wrap:anywhere]", allocation.amount_minor > 0 ? "text-positive" : "text-destructive")}>{formatSignedCurrency(allocation.amount_minor, goal.currency_code)}</span></div>)}</div>}</section>
